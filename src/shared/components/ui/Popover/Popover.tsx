...
function PopoverInternalProvider({
  placement,
  className,
  style,
  children,
}: { placement: string; children: React.ReactNode } & StyleProps) {
  ...
  useEffect(() => {
    ...
    switch (placement) {
      case "top": {
        ...
        left = Math.max(
          margin,
          Math.min(left, window.innerWidth - content.width - margin),
        );
        pos.left = left;
        break;
      }
      ...
      case "left": {
        ...
        top = Math.max(
          margin,
          Math.min(top, window.innerHeight - content.height - margin),
        );
        pos.top = top;
